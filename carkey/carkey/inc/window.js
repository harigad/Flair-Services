$flair.window = {
	...
	printPage: function(str,noFade,hideHeader) {	
		...
		if(hideHeader){
			 $flair.header.hideHeaderBg();
		}else{
			 $flair.header.showHeaderBg();
		}
	...
	print: function(divName,str) {		
	  ...
	  printFromDiv: function(page,response){
							var str = $('#' + page).html();
							
							if(response){
								for (variable in response){
								    var find = "__" + variable + "__";
									str = str.replace((new RegExp(find, 'g')),response[variable]);								
								}
							}
		
		if(response && response.showTitle){		 
		    var t =  response.showTitle; 
			$flair.header.setLogo(t);
			this.printPage(str);
		}else{
		   this.printPage(str,true,true);
		}
