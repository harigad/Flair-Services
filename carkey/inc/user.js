$flair.user = {
	...
	begin: function(title,id){
			...
			$flair.window.printPage('<div style="vertical-align:top;" ><div style="background-color:#eee;vertical-align:top;text-align:left;margin-left:0px;margin-bottom:0px;display:inline-block;position:relative;top:0px;" ><img src="' + $flair.go.request("photo") + '" style="width:320px;"  ></div><div id="left_user_content_holder_main"  ><div class="loading" style="height:40px;background-position:center;background-repeat:no-repeat;" ></div></div>',true);
		
			if($flair.login.user.id==id){
			  that.user = $flair.login.user;
			  that.print();
			}else if(that.users[id]){		
	...
	print: function(){	
		$flair.window.print("left_user_content_holder_main",$flair.profile.print(this.user));			
	},
