<?php
	...
	$_new_users_sql_obj = $db->selectRows("select feed.created as updated,user.id, user.name,user.photo_big,place.pid,place.name as place,place.city,place.lat,place.lng from role 
	inner join user on role.uid = user.id 
	inner join place on role.pid = place.pid 
	inner join feed on user.id = feed.recipient 
	" . $where . " order by feed.created desc limit 3");
